...
module.exports.get_list_live_sound_ios = async function (req, res) {
    try {
        ...
        for (let i = 0; i < data.length; i++) {
            ...
            //     continue;
            // }
            data[i].label = data[i].label_?.find(
...
module.exports.get_list_meditation_ios = async function (req, res) {
    try {
        ...
        for (let i = 0; i < data.length; i++) {
            ...
            data[i].poster = data[i].poster_?.find(
                (item) => item.language === "ru"
            )?.value;
            data[i].text = data[i].text_?.find(
                (item) => item.language === "ru"
            )?.value;
        }
        res.status(201).json({
    ...
    try {
        ...
        for (let i = 0; i < data.length; i++) {
            if (!(!data[i].access || data[i].access.indexOf(access) !== -1)) {
                if (
                    data[i].access.length === 1 &&
                    data[i].access.indexOf("VIP") !== -1
                ) {
                    data.splice(i, 1);
                    continue;
                }
    ...
    try {
        ...
        for (let i = 0; i < data.length; i++) {
            ...
            if (check.available_courses) {
                ...
                data[i].avalibel =
                    avalibel !== undefined && avalibel.end_date >= new Date();
...
module.exports.answer_question_test = async function (req, res) {
    try {
        ...
        let answers_ok_is =
            question_data?.answers?.findIndex(
        ...
        let status_answer = question_data?.answers?.find(
            (item) => item.label === answer.label
        );
        let status = true;
        if (answers_ok_is) {
...
module.exports.complete_test = async function (req, res) {
    try {
        ...
        result.status = true;
        const resultation = test.result?.find(
            (item) => item.start_balls <= balls && balls <= item.end_balls
        );
        result.result = {
...
module.exports.get_v2_list_audio_ios = async function (req, res) {
    try {
        ...
        // }
        let access =
            check._id && check.access ? check.access : "Без регистрации";
