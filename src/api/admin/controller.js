...
module.exports.login = async function (req, res) {
    ...
    if (candidate) {
        ...
        if (
            passwordResult &&
            (candidate.type_admin === "Администратор" ||
                candidate.type_admin === "Музыкотерапевт")
        ) {
            // const token = jwt.sign({
...
module.exports.get_list_live_sound = async function (req, res) {
    try {
        ...
        const count_page =
            Math.ceil(
                (await LiveSound.find(filter).count()) / limitPageDataVeb
            ) - 1;
        res.status(201).json({
            data,
...
module.exports.get_list_audio = async function (req, res) {
    try {
        ...
        if (req.params.genre && req.params.genre !== "null")
            filter.genre = req.params.genre;
        // const data = await Audio.find(filter, null, { skip: page, limit: limitPageDataVeb });
        const data = await Audio.find(filter);
        const count_page =
            Math.ceil((await Audio.find(filter).count()) / limitPageDataVeb) -
            1;
        ...
        for (let i = 0; i < data?.length; i++) {
            ...
            data[i].like_tooltip = await LikeAudio.find(
...
module.exports.get_list_video = async function (req, res) {
    try {
        ...
        const count_page =
            Math.ceil((await Video.find(filter).count()) / limitPageDataVeb) -
            1;
        ...
        for (let i = 0; i < data?.length; i++) {
            ...
            data[i].like_tooltip = await LikeVideo.find(
                { id_root: data[i]._id.toString() },
                { date: 1, user_name: 1 }
            );
            data[i].like = data[i].like_tooltip?.length;
...
module.exports.get_list_maps = async function (req, res) {
    try {
        ...
        const count_page =
            Math.ceil((await Maps.find(filter).count()) / limitPageDataVeb) - 1;
...
module.exports.get_list_test = async function (req, res) {
    try {
        ...
        const count_page =
            Math.ceil((await Test.find(filter).count()) / limitPageDataVeb) - 1;
...
module.exports.get_list_courses = async function (req, res) {
    try {
        ...
        const count_page =
            Math.ceil((await Courses.find(filter).count()) / limitPageDataVeb) -
            1;
        ...
        for (let i = 0; i < data.length; i++) {
            ...
            if (
                data[i].info_tooltip &&
                data[i].info_tooltip?.length &&
                data[i].length_lessons !== 0
            ) {
...
module.exports.get_list_meditation = async function (req, res) {
    try {
        ...
        const data = await Video.find(
            { category: "meditation", ...filter },
            null,
            { skip: page, limit: limitPageDataVeb }
        );
        const count_page =
            Math.ceil(
                (await Video.find({
...
module.exports.get_list_classic = async function (req, res) {
    try {
        ...
        const filter = req.params.label
            ...
            : {};
        const data = await Video.find(
            { category: "classic", ...filter },
            null,
            { skip: page, limit: limitPageDataVeb }
        );
        const count_page =
            Math.ceil(
                (await Video.find({ category: "classic", ...filter }).count()) /
                    limitPageDataVeb
            ) - 1;
        res.status(201).json({
            data,
...
module.exports.users_sort = async function (req, res) {
    try {
        ...
        if (req.body.data.is_admin && req.body.data.is_admin !== "null")
            filter.is_admin = req.body.data.is_admin === "true";
        if (req.body.data.access && req.body.data.access !== "null")
            filter.access = req.body.data.access;
        if (req.body.data.language && req.body.data.language !== "null")
            filter.language = req.body.data.language;
        ...
        const count_page =
            Math.ceil((await User.find(filter).count()) / limitPageDataVeb) - 1;
...
module.exports.get_list_user_fin = async function (req, res) {
    try {
        ...
        let filter = req.params.full_name
            ? req.params.full_name !== "null"
                ? { fullName: { $regex: req.params.full_name } }
                : {}
            : {};
        filter["musicTherapy.id"] = check.id;

        if (req.params.is_admin && req.params.is_admin !== "null")
            filter.type_admin = req.params.is_admin;
        if (req.params.access && req.params.access !== "null")
            filter.access = req.params.access;
        if (req.params.language && req.params.language !== "null")
...
module.exports.delete_test = async function (req, res) {
    try {
        ...
        if (delete_dop.length)
            await QuestionsTest.remove({ course_id: req.body._id });
...
module.exports.delete_course = async function (req, res) {
    try {
        ...
        if (delete_data) await delete_data.delete();
        if (delete_dop.length)
            await LessonsCourses.remove({ course_id: req.body._id });
        res.status(201).json("OK");
...
module.exports.re_translation_admin = async function (req, res) {
    try {
        ...
        if (translations) {
            Object.entries(req.body.data).forEach((item) => {
                translations[item[0]] =
                    item[1] && item[1].length ? item[1] : null;
            });
        } else {
            translations = new Translation({
                root: 1,
...
module.exports.playlist_re = async function (req, res) {
    try {
        ...
        await customAccess.customAccess(
            new_data,
            "playlist",
            req.body.data.custom_access
        );
...
module.exports.get_playlist = async function (req, res) {
    try {
        ...
        for (let i = 0; i < data.length; i++) {
            data[i].custom_access = await customAccess.customAccessGet(
                data[i],
                "playlist"
            );
            console.log("data[i].custom_access", data[i].custom_access);
            data[i].audio_l = data[i].ids_audio ? data[i].ids_audio.length : 0;
            data[i].video_l = data[i].ids_video ? data[i].ids_video.length : 0;
        }
        const count_page =
            Math.ceil(
                (await Playlist.find(filter).count()) / limitPageDataVeb
            ) - 1;
